...
//  BSideBack
//
//  Created by Lucie Grunenberger  on 12/12/2025.
//
...
struct BadgeController: RouteCollection {
    func boot(routes: any RoutesBuilder) throws {
        let badges = routes.grouped("badge")
        let protected = badges.grouped(JWTMiddleware())
        
        protected.get(use: getAllBadges)
...
//        }
    ...
    @Sendable
    func getAllBadges(_ req: Request) async throws -> [BadgeResponseDTO]{
        try req.auth.require(UserPayload.self)
        let badges = try await Badge.query(on: req.db).all()
        
                return badges.map { $0.toDTO() }
    }
